...
int main(int argc, char* argv[])
{
	...
	// Read the root certs from the firmware binary
	if (ReadRootCertificate(image_path) != 1)
	{
		goto _END_1;
	}
	DumpRootCerts();
	...
	if(M2M_SUCCESS == builder_init(output_path,"wb")) {
		...
		M2M_PRINT("Unable to initialize output file.\n");
		goto _END_2;
	}
	...
	// TODO: Since we're building off an existing firmware, need to figure out correct offset
...
#if(ENABLE_WRITE_TLS_SERVER_SECTION == FALSE)
